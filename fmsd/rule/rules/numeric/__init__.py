...
rule_absorption_add_pos = MatchRule(
    NEG_INFINITY < x,
    (INFINITY + x) @ EQ @ INFINITY, equiv=False)
rule_absorption_add_neg = MatchRule(
    x < INFINITY,
    (NEG_INFINITY + x) @ EQ @ NEG_INFINITY, equiv=False)
...
rule_absorption_sub_neg = MatchRule(NEG_INFINITY < x, (NEG_INFINITY - x) @ EQ @ NEG_INFINITY,
                                    equiv=False)
...
rule_base_div_one = MatchRule(((NEG_INFINITY < x) & (x < INFINITY) & (x @ NEQ @ ZERO)),
                              (x / x) @ EQ @ ONE, equiv=False)
...
rule_mul_div_id = MatchRule(((NEG_INFINITY < y) & (y < INFINITY)) & (y @ NEQ @ ZERO),
                            ((x / y) * y) @ EQ @ x,
                            equiv=False)
rule_annihilation_pos = MatchRule((NEG_INFINITY < x) & (x < INFINITY), (x / INFINITY) @ EQ @ ZERO,
                                  equiv=False)
rule_annihilation_neg = MatchRule((NEG_INFINITY < x) & (x < INFINITY),
                                  (x / NEG_INFINITY) @ EQ @ ZERO, equiv=False)
...
rule_translation = MatchRule((NEG_INFINITY < x) & (x < INFINITY),
                             ((x + y) < (x + z)) @ EQ @ (y < z), equiv=False)
...
rule_distributivity_max_pos = MatchRule(x >= ZERO,
                                        (x * (y @ MAX @ z)) @ EQ @ ((x * y) @ MAX @ (x * z)))
rule_distributivity_max_neg = MatchRule(x <= ZERO,
                                        (x * (y @ MAX @ z)) @ EQ @ ((x * y) @ MIN @ (x * z)))
rule_distributivity_min_pos = MatchRule(x >= ZERO,
                                        (x * (y @ MIN @ z)) @ EQ @ ((x * y) @ MIN @ (x * z)))
rule_distributivity_min_neg = MatchRule(x <= ZERO,
                                        (x * (y @ MIN @ z)) @ EQ @ ((x * y) @ MAX @ (x * z)))
