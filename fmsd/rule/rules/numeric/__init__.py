...
rule_cancellation_add = MatchRule(
    (NEG_INFINITY < x) & (x < INFINITY),
    ((x + y) @ EQ @ (y + z)) @ EQ @ (y @ EQ @ z),
    equiv=False,
)
rule_absorption_add_pos = MatchRule(
    NEG_INFINITY < x, (INFINITY + x) @ EQ @ INFINITY, equiv=False
)
rule_absorption_add_neg = MatchRule(
    x < INFINITY, (NEG_INFINITY + x) @ EQ @ NEG_INFINITY, equiv=False
)
...
rule_cancellation_sub = MatchRule(
    (NEG_INFINITY < x) & (x < INFINITY),
    (((x - y) @ EQ @ (y - z)) @ EQ @ (y @ EQ @ z)),
    equiv=False,
)
rule_inverse = MatchRule(
    (NEG_INFINITY < x) & (x < INFINITY), (x - x) @ EQ @ ZERO, equiv=False
)
rule_absorption_sub_pos = MatchRule(
    x < INFINITY, (INFINITY - x) @ EQ @ INFINITY, equiv=False
)
rule_absorption_sub_neg = MatchRule(
    NEG_INFINITY < x, (NEG_INFINITY - x) @ EQ @ NEG_INFINITY, equiv=False
)

rule_base_mul = MatchRule(
    (NEG_INFINITY < x) & (x < INFINITY), (x * ZERO) @ EQ @ ZERO, equiv=False
)
...
rule_cancellation_mul = MatchRule(
    ((NEG_INFINITY < x) & (x < INFINITY)) & x @ NEQ @ ZERO,
    ((x * y) @ EQ @ (y * z)) @ EQ @ (y @ EQ @ z),
    equiv=False,
)
rule_absorption_mul_pos = MatchRule(
    ZERO < x, (x * INFINITY) @ EQ @ INFINITY, equiv=False
)
rule_absorption_mul_neg = MatchRule(
    ZERO < x, (x * NEG_INFINITY) @ EQ @ NEG_INFINITY, equiv=False
)

rule_identity_div = MatchRule(x / ONE, x)
rule_base_div_zero = MatchRule(x @ NEQ @ ZERO, (ZERO / x) @ EQ @ ZERO, equiv=False)
rule_base_div_one = MatchRule(
    ((NEG_INFINITY < x) & (x < INFINITY) & (x @ NEQ @ ZERO)),
    (x / x) @ EQ @ ONE,
    equiv=False,
)
...
rule_mul_div_id = MatchRule(
    ((NEG_INFINITY < y) & (y < INFINITY)) & (y @ NEQ @ ZERO),
    ((x / y) * y) @ EQ @ x,
    equiv=False,
)
rule_annihilation_pos = MatchRule(
    (NEG_INFINITY < x) & (x < INFINITY), (x / INFINITY) @ EQ @ ZERO, equiv=False
)
rule_annihilation_neg = MatchRule(
    (NEG_INFINITY < x) & (x < INFINITY), (x / NEG_INFINITY) @ EQ @ ZERO, equiv=False
)

rule_base_pow = MatchRule(
    (NEG_INFINITY < x) & (x < INFINITY), (x**ZERO) @ EQ @ ONE, equiv=False
)
...
rule_translation = MatchRule(
    (NEG_INFINITY < x) & (x < INFINITY), ((x + y) < (x + z)) @ EQ @ (y < z), equiv=False
)
rule_scale = MatchRule(
    (ZERO < x) & (x < INFINITY), ((x * y) < (x * z)) @ EQ @ (y < z), equiv=False
)
