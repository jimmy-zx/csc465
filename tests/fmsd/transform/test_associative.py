...
def test_commutative_binary(op):
    ...
    assert trf.verify(op(a, op(b, c)), op(op(a, b), c)) == issubclass(op, AssociativeOperator)
    assert trf.verify(op(op(a, b), c), op(a, op(b, c))) == issubclass(op, AssociativeOperator)
    assert trf.verify(op(a, op(b, op(c, d))), op(op(a, b), op(c, d))) \
           == issubclass(op, AssociativeOperator)
    assert trf.verify(op(a, op(b, op(c, d))), op(op(op(a, b), c), d)) \
           == issubclass(op, AssociativeOperator)
...
def test_commutative_numeric(op):
    trf = AssociativeTransform()
    assert trf.verify(op(x, op(y, z)), op(op(x, y), z)) == issubclass(op, AssociativeOperator)
    assert trf.verify(op(op(x, y), z), op(x, op(y, z))) == issubclass(op, AssociativeOperator)
    assert trf.verify(op(w, op(x, op(y, z))), op(op(w, x), op(y, z))) \
           == issubclass(op, AssociativeOperator)
    assert trf.verify(op(w, op(x, op(y, z))), op(op(op(w, x), y), z)) \
           == issubclass(op, AssociativeOperator)
