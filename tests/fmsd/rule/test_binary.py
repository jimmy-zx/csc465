...
def test_duality():
    assert ExpressionTransform(rule_duality_and).verify(Flip(And(a, b)), Or(Flip(a), Flip(b)))
    assert ExpressionTransform(rule_duality_or).verify(Flip(Or(a, b)), And(Flip(a), Flip(b)))


def test_exclusion():
    assert ExpressionTransform(rule_contrapositive).verify(Implies(a, b), Implies(Flip(b), Flip(a)))
    assert ExpressionTransform(rule_exclusion).verify(Equals(a, Flip(b)), NotEquals(a, b))
    assert ExpressionTransform(rule_exclusion).verify(NotEquals(a, b), Equals(a, Flip(b)))


def test_material_implication():
    assert ExpressionTransform(rule_material_implication).verify(Implies(a, b), Or(Flip(a), b))
    assert ExpressionTransform(rule_material_implication).verify(Or(Flip(a), b), Implies(a, b))


def test_inclusion():
    assert ExpressionTransform(rule_inclusion_and).verify(Implies(a, b), Equals(And(a, b), a))
    assert ExpressionTransform(rule_inclusion_and).verify(Equals(And(a, b), a), Implies(a, b))
    assert ExpressionTransform(rule_inclusion_or).verify(Implies(a, b), Equals(Or(a, b), b))
    assert ExpressionTransform(rule_inclusion_or).verify(Equals(Or(a, b), b), Implies(a, b))
